import {APILimitExceededError, getValidInfo, Language, WordInfo, WordService} from "./WordService";
    ...
    async process(word: Word, infoWanted: number): Promise<void> {
        ...
        if (infoWanted & WordInfo.trans) {
            ...
            const response = await axios.get(url).catch(error => {
                if (error.response && error.response.data.message === 'The Linguee server returned 503') {
                    throw new APILimitExceededError('Linguee API limit exceeded!');
                }
            });
            if (response) {
        ...
        if (infoWanted & WordInfo.transSens + WordInfo.sens) {
            ...
            if (response) {
                ...
                if (infoWanted & WordInfo.sens) {
                    ...
                    let sens: string[] = result.map(item => Linguee.tidy(item.src));
                ...
                if (infoWanted & WordInfo.transSens) {
                    if (result.length > 0) {/* case where translation fails but transSens works*/
                        if (lingueeTranslations.length === 0) {
                            lingueeTranslations.push(new Translation());
                        }
                        const transSens: TranslatedSentence[] = [];
                        for (let item of result) {
                            const sp: TranslatedSentence = {src: Linguee.tidy(item.src), dst: Linguee.tidy(item.dst)};
                            if (this.dstlang === Language.zh) {
                                const converter = require('opencc-js').Converter({'from': 'hk', to: 'cn'});
                        ...
                        for (let prevTrans of word.possTranslations) {
                            if (prevTrans.transSens) prevTrans.transSens.push(...transSens);
